...
1. クライアントID: 任意のID(例: demo)を設定します。

2. クライアントプロトコル: openid-connect を選択します。

3. 有効なリダイレクト URI: シラサギの認証用 URL を指定します (例: https://[シラサギURL]/.mypage/login/oid/[ファイル名]/callback)。
   環境によりパスが異なる場合があるため、シラサギのシステム設定画面で確認してください。

4. クライアント認証: サーバー間通信を行うため ON にし、クライアントシークレットを発行・保存します。 

5. ユーザー、グループを作成し、任意にパスワードを設定します。
